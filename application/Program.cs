...
// --- TỰ ĐỘNG TẠO TÀI KHOẢN ADMIN/STAFF NẾU CHƯA CÓ ---
using (var scope = app.Services.CreateScope())
{
    try 
    {
        ...
        {
            var existing = await db.Users.FirstOrDefaultAsync(x => x.Username == u.Username);
            if (existing == null)
            {
                db.Users.Add(u);
        ...
        int slotsCreated = await scheduler.AutoGenerateSlotsAsync(7);
        if (slotsCreated > 0)
        {
    ...
    catch (Exception ex)
    {
        ...
        Console.WriteLine("👉 M hãy kiểm tra SQL Server đã chạy chưa và chuỗi kết nối trong appsettings.json nhé!");
    }
}
